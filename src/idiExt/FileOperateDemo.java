...
public class FileOperateDemo {
	...
	private static boolean copyFile(String srcPath, String destDir) {
		...
		//包含处理内部类逻辑
		FilenameFilter fileFilter=new InnerFileFilter(fileName);
		String[] nameArray= new File(srcDir).list(fileFilter);

		for (String fileName2:nameArray) {
			//更改原文件路径
			srcPath=srcPath.substring(0,srcPath.lastIndexOf("/"))+"/"+fileName2;
			String destPath = destDir+ "/"+ fileName2;
			if(fileName2.indexOf("$")>=0) System.out.println(srcPath);
